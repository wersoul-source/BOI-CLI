...
func (m *WizardModel) finish() (tea.Model, tea.Cmd) {
	...
	m.done = true
	m.cancelled = false
	m.infoMsg = fmt.Sprintf("✓ %d provider(s) ready to save; run 'boi provider qualify' before Agent use", len(m.providers))
	return m, nil
}
...
func (m *WizardModel) viewDone() string {
	...
	sb.WriteString("\n\n  " + successMsgStyle.Render("✓ Provider configuration ready"))
	sb.WriteString("\n  " + infoMsgStyle.Render("Press any key to save securely and exit"))
...
func Run(r *registry.Registry) *WizardResult {
	...
	model, err := p.Run()
	if err != nil {
		return &WizardResult{Cancelled: true, Err: err}
	}
	if wm, ok := model.(*WizardModel); ok {
		if res := wm.Result(); res != nil {
			if !res.Cancelled && len(res.Providers) > 0 {
				res.EnvPath, res.BackupPath, res.Err = writeProviderEnv(wm.providers)
			}
			return res
		}
	}
